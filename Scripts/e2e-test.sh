#!/usr/bin/env bash
#
# End-to-end test: sends known MIDI into the real receivemidi binary through a
# virtual MIDI port (using sendmidi) and checks that receivemidi prints the
# expected text. This exercises the whole path - the OS MIDI stack, message
# decoding and text formatting - which the in-process unit tests can't.
#
# Usage: e2e-test.sh <path-to-receivemidi> [path-to-sendmidi]
#
# Needs virtual MIDI ports (macOS and Linux only) and a sendmidi binary
...
RM="${1:?usage: e2e-test.sh <path-to-receivemidi> [path-to-sendmidi]}"
SM="${2:-sendmidi}"
...
if ! command -v "$SM" >/dev/null 2>&1 && [ ! -x "$SM" ]; then
    echo "skip: sendmidi not found (needed to send the messages)"
    exit 0
fi

# receivemidi (the binary under test) owns one long-lived virtual input port and
# prints notes as numbers; sendmidi connects to it and sends every message type
# checked below.
PORT="RM-E2E-$$"
OUT="$(mktemp)"
...
RESP_OUT="$(mktemp)"
INIT_OUT="$(mktemp)"
# responder (the binary under test): manager channel 2, 3 members, with
# distinct optional-feature flags
